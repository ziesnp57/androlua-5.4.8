...
static struct zip_source* S_create_source_file(lua_State* L, struct zip* ar) {
    ...
    int                start = lua_gettop(L) < 5 ? 0  : luaL_checkinteger(L, 5);
    int                len   = lua_gettop(L) < 6 ? -1 : luaL_checkinteger(L, 6);
...
static struct zip_source* S_create_source_zip(lua_State* L, struct zip* ar) {
    ...
    int                file_idx = luaL_checkinteger(L, 5);
    int                flags    = lua_gettop(L) < 6 ? 0  : luaL_checkinteger(L, 6);
    int                start    = lua_gettop(L) < 7 ? 0  : luaL_checkinteger(L, 7);
    int                len      = lua_gettop(L) < 8 ? -1 : luaL_checkinteger(L, 8);
...
static int S_OR(lua_State* L) {
    ...
    int top = lua_gettop(L);
    while ( top ) {
        result |= luaL_checkinteger(L, top--);
    }
