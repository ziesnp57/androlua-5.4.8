  ...
  while entry do
    ...
    elseif name:find("META%-INF") then
      ...
      if entry.getName() == "AndroidManifest.xml" then
        ...
        local s=f:read("a")
        f:close()
        s=string.gsub(s,touint32(code),touint32(tointeger(appcode) or 1))
        s=string.gsub(s,touint32(21),touint32(tointeger(appsdk) or 15))
...
local function bin_noBuild(path)
  local p={}
